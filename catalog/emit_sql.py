...
def main() -> int:
    ...
    shop_id = parts["suppliers"][0]["supplier_id"] if parts["suppliers"] else "(none)"
    sql = [
        f"-- Real parts for Solar Builder.  VERSION {STAMP}",
        ...
        "--",
        f"-- BEFORE RUNNING, CHECK LINE 3 OF THIS HEADER: our shop must be {shop_id!r}.",
        "-- A version of this file once inserted the shop as supplier S1, the same id",
        "-- as a demo shop that this file deletes at the end, so the shops ended up",
        "-- referenced by their own listings and the transaction aborted. If your file",
        "-- inserts S1, it is the old one. Use the newest.",
        "",
        "BEGIN;",
        "",
